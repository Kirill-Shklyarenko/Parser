    ...
    for frame in telemetry:
                ...
                if pm_pk_cand:
                    ...
                    if candidate['state'] == 4:
                        ...
                        for air_marks_upd_req in frame_reader.air_marks_update_requests():
                            ...
                            beam_task_pk_cand_if_state_4 = db.get_pk_b_tasks_candidates(air_marks_upd_req['markId'],
                                                                                        candidate['taskId'],
                                                                                        candidate['antennaId'], 2)
                            if beam_task_pk_cand_if_state_4 is None:
                                log.debug(f'BeamTask PK if state == 4 : {beam_task_pk_cand}')
                                beam_task_pk_cand_if_state_4 = beam_task_pk_cand
                            primary_mark_pk_if_state_4 = db.get_pk_primary_marks(
                                {'BeamTask': beam_task_pk_cand_if_state_4,
                                 'primaryMarkId': candidate['primaryMarkId']})
                            candidate_history_pk_if_state_4 = db.get_pk_cand_hists_if_state_4(
                                {'BeamTask': beam_task_pk_cand_if_state_4,
                                 'PrimaryMark': primary_mark_pk_if_state_4})
        ...
        for air_track in frame_reader.air_tracks():
            ...
            if beam_task_pk:
                ...
                log.debug(f'Get BeamTask PK if state == 4 : {beam_task_pk_cand_if_state_4}')
                air_track.update({'BeamTask': beam_task_pk_cand_if_state_4})
                air_track.update(
                    {'pulsePeriod': db.read_from_table('BeamTasks', {'BeamTask': air_track['BeamTask']})[0][7]})
                log.debug(f'PrimaryMark PK if state == 4 : {primary_mark_pk_if_state_4}')
                air_track.update({'PrimaryMark': primary_mark_pk_if_state_4})
                air_track.update({'scanTime': db.read_from_table('PrimaryMarks',
                                                                 {'PrimaryMark': air_track['PrimaryMark']})[0][3]})
                candidate_history_pk = db.get_pk_cand_hists(air_track['BeamTask'], air_track['PrimaryMark'])
                air_track.update({'CandidatesHistory': candidate_history_pk})
